...
#									&& ($cp->circ_modifier !~ /COMPANY|ILL-RET|MISCELLANEOUS|IPOD/) ){
                                                                ...
                                                                if ( ( $cp->circ_lib < 22 )
                                                                        ...
                                                                        && ($cp->circ_modifier !~ /OTHER/)
                                                                        && ( !(($cp->circ_modifier eq "AUDIOBOOK") && ($cn->label =~ /^CDTE/)) ) #NDL
                                                                        && ($cp->circ_modifier !~ /21DGEN|7DMAG/)				 #GRPL
									&& ($cp->circ_modifier !~ /COMPANY|ILL-RET|MISCELLANEOUS|IPOD/) 	 #BDL
                                                                ){
                                                                        my $circ_mod=$cp->circ_modifier;

                                                                        if ( ($cp->circ_lib == 21)
                                                                                  && ( ($cn->label =~ /^DVD/)
                                                                                    ||  (($cp->circ_modifier eq "AUDIOBOOK") && ($cn->label =~ /^CD/))  
										    ||  ($cp->circ_modifier eq "AUDIOBOOK-CD") ) ){
                                                                               $circ_mod = "non-circulating";
                                                                        }

									if ( (($cp->circ_modifier eq "21D") && ($shelves{$cp->location}->name eq "New")) 
										|| ($shelves{$cp->location}->name eq "Express") 
										|| ($cp->circ_modifier eq "42D")
										|| (($cp->circ_lib == 10) && ($cn->label =~ /^USB|^Furn\.\sColl\./)) ) {
										$circ_mod = "DNC";	
									}
